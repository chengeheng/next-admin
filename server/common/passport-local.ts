...
const opts = {
  ...
  secretOrKey: config.JWT_KEY, // Verify the issuer.
  issuer: config.JWT_ISSUER, // Verify the audience.
  audience: config.JWT_AUDIENCE, // Enable only the HS256 algorithm.
  algorithms: [config.JWT_ALG], // Pass the request object back to the callback so we can attach the JWT to it.
